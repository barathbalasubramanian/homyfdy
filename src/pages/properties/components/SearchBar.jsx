...
function SearchBar({ setpropertyName, propertyNames }) {
  ...
  const handleSelectProperty = (property) => {
    setInputValue(property);         // Set the selected property as input value
    setFilteredProperties([]);       // Clear the filtered properties (hide dropdown)
    setpropertyName(property);       // Trigger the setpropertyName function if needed
  };
        ...
        <div className="w-11/12 flex rounded-xl " style={{ backgroundColor: "#141414", padding: "6px 10px" }}>
            <div className="flex-1 shrink self-stretch my-auto text-xl basis-10 text-stone-500 max-md:max-w-full">
              <input
                ...
                className="w-full outline-none border-none max-md:text-sm"
