...
function CompareDiv({setCmpPage,CmpCnt,SetCmpBtn}) {
    ...
    <div className='bg-white rounded-xl px-6 py-4 flex flex-col gap-3 items-start justify-between w-[25em] max-md:w-[18em]'>
        ...
        <div className='flex w-full items-center px-6 justify-between'>
            ...
            <div onClick={() => CmpCnt >= 2 && navigate("/compare")} className={`mt-4 w-3/6 text-center text-white justify-center flex items-center gap-3 px-2 py-1 rounded ${CmpCnt >= 2 ? 'bg-green-500 cursor-pointer opacity-100' : 'bg-green-500 cursor-not-allowed opacity-50'}`} >
                <div>Compare</div>
            </div>
        </div>
    </div>
