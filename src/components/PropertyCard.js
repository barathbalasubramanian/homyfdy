...
function PropertyCard({verbose}) {
  const navigate = useNavigate();
  return (
    <div className={`prop-con ${verbose ? 'bg-white' : ''}`} > 
      ...
      <div className='flex items-center justify-between'>
        <div>
            ...
            <div className={`${!verbose ? 'text-white' : 'text-black'}`}>$550,000</div>
        </div>
        <div onClick={()=>navigate('/propertydetails')} className='cursor-pointer border-none flex items-center w-fit bg-green-600 shadow-[0px_0px_21px_rgba(31,200,39,1)] gap-3 px-2 py-2' style={{fontSize:"13px",textWrap:"nowrap",backgroundColor:"var(--green)",borderRadius:"5px"}}>
